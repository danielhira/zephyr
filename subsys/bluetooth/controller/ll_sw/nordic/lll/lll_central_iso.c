...
static int prepare_cb(struct lll_prepare_param *p)
{
	...
	do {
		...
		if (link) {
			...
			} else if (tx->payload_count >= (payload_count +
							 cis_lll->tx.bn)) {
				link = NULL;
			} else {
				if (tx->payload_count != payload_count) {
	...
	if (false) {
		...
		radio_pkt_tx_set(radio_ccm_tx_pkt_set(&lll->ccm_tx, pdu_tx));
#endif
...
static void isr_tx(void *param)
{
	struct lll_conn_iso_stream *cis_lll = param;
